...
async function handler(req, res) {
  const currentMode = req.query.currentMode || 'basement';
  ...
  try {
    ...
    // 2. Fetch rows (excluding the PIN in column H for security)
    const getRows = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${targetSheetTitle}!A2:G`
    });
    ...
    console.error('API Error in get-complaints:', error);
