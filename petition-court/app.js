...
document.addEventListener('DOMContentLoaded', () => {
    ...
    localStorage.removeItem('ansan_petition_court_roster_data');
    ...
    async function loadSavedData() {
        try {
            const res = await fetch(`/api/get-petition-court`);
            if (res.ok) {
                const data = await res.json();
                if (data.success) {
                    roster = data.roster;
        ...
        const storageKey = `ansan_petition_court_roster_data`;
    ...
    rosterTbody.addEventListener('click', async (e) => {
        ...
        if (btn) {
            ...
            try {
                const res = await fetch('/api/delete-petition-court', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                <article class="a4-page">
                    ...
                    <div style="font-size: 8.5pt; line-height: 1.45; border: 1px solid #111; padding: 10px; margin-bottom: 12px; background-color: #fcfcfc;">
                        <strong style="display: block; font-size: 9.5pt; text-align: center; margin-bottom: 6px; border-bottom: 1px solid #ccc; padding-bottom: 4px;">
                            [안산유통상가 임시관리인 선임결정 취소 촉구 탄원 연명부]
                        </strong>
                        안산유통상가의 임시관리인 선임결정 취소와 새로운 관리체계의 조속한 회복을 위해 구분소유자 및 입점자·관계인 분들의 적극적인 서명 동참을 부탁드립니다.<br>
                        <strong>■ 탄원 목적:</strong> 법원에 임시관리인 선임 결정을 취소하여 새 관리인이 정상적인 관리업무를 수행할 수 있도록 선처를 청원함.<br>
